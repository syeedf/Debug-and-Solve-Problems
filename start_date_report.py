...
def get_same_or_newer(start_date, sorted_data):
...
def list_newer(start_date):
  sorted_data= preprocess()

  while start_date < datetime.datetime.today():

    start_date, employees = get_same_or_newer(start_date, sorted_data)
...
def preprocess():
    ...
    reader = csv.reader(data[1:])  # skip header
    sorted_data = sorted(reader, key=operator.itemgetter(3))
    return sorted_data
